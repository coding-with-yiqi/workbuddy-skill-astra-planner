...
```bash
git clone https://github.com/coding-with-yiqi/workbuddy-skill-astra-planner.git \
...
Codex 要连 `chatgpt.com`。脚本**不预设任何代理地址**——不同机器端口不同，写死默认值只会误导。没设置会直接报错退出，不会替你猜。

```bash
# 二选一即可，建议写进 shell profile
export ASTRA_PROXY=http://<你的代理地址>:<端口>
export HTTPS_PROXY=http://<你的代理地址>:<端口>     # 标准变量，同样被识别
```
